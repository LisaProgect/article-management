...
export class EnhancedWithAuthHttpService {
	...
	public async post<T, TD>(url: string, data: TD, config: IHttpConfig = {
	},): Promise<T> {
		return this.httpService.post<T, TD>(url, data, await this.attachAuthHeader(config,),)
	}

	public async put<T, TD>(url: string, data: TD, config: IHttpConfig = {
	},): Promise<T> {
		return this.httpService.put<T, TD>(url, data, await this.attachAuthHeader(config,),)
	}

	public async patch<T, TD>(url: string, data: TD, config: IHttpConfig = {
	},): Promise<T> {
	...
	private async attachAuthHeader(config: IHttpConfig,): Promise<IHttpConfig> {
		return {
			...config,
			headers: {
				...config.headers, ...(await this.populateTokenToHeaderConfig()),
			},
		}
	}
